...
function save(){
    if (getAllNote().length>0){
        ...
        alert("ERROR","There aren't annotation.","It's not possible to save the annotations");
    }
}
...
function addSavedNoteFragment(viewNote){
    ...
    span.setAttribute("data-content", "<span class='popoverNote'><p>Label: " + viewNote.bodyLabel + "</p><p>Mail: " + viewNote.email + "</p><p>Type: " + viewNote.noteLabel + "</p><p>Author: " + viewNote.name + "</p><p>Date: " + formattingTime(viewNote.time) + "</p></span>");
...
function addNoteTabUnsaved(viewNote, myTypeNote){
    var strCom = viewNote.bodyLabel;
    if (strCom.length > 15){
        strCom = strCom.substring(0,15) + " ...";
    }
    var myId = "li-" + viewNote.id;
    var strEditButton = "<span id='glyphiconEdit-"+ viewNote.id +"' style='cursor: pointer;' class=\"input-group-addon\"><span  class=\"glyphicon glyphicon-edit\"></span></span>";
    var strRemoveButton = "<span id='glyphiconRemove-"+ viewNote.id +"' style='cursor: pointer;'  class=\"input-group-addon\"><span  class=\"glyphicon glyphicon-remove\"></span></span>";
    $("#unsaved-notes ul").prepend("<li id='" + myId + "' class='list-group-item no-border'><strong>" + viewNote.noteLabel + "</strong><br>" + strCom + "<br><span class='small'>" + formattingTime(viewNote.time) + "&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;" + myTypeNote  + "</span>"+strEditButton+strRemoveButton+"</li>");
...
function setEditDataIstance(theLabel,theResource){
    ...
    if (theResource!==undefined && theResource.length>0) $("#instance-list").val(theResource);
}
